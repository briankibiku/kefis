      ...
      <template v-slot:large-screen>
        <div class="center-align-item d-flex" v-if="isTeamMember">
          <div>
            ...
            <a href="/tournament" class="subheading2">Go Back</a>
          </div>
        </div>
        <div class="center-align-item d-flex" v-if="!isTeamMember">
          <div>
              ...
              <div class="d-flex flex-direction-row justify-content-between">
                ...
                >
                  <div>
                    ...
                    <div class="subheading3 text-left">
                      Team Leader: {{ tournament.team_leader }}
                    </div>
                  </div>
                </div>
                <ConfirmationModal
                  ...
                  @click="joinTeam(tournament.phone)"
                />
              </div>
              <div class="divider"></div>
            </div>
  ...
  methods: {
    ...
    async isAlreadyTeamMember() {
      let isTeamMemberResponse = await this.$axios.$get(
        `/apiproxy/team-play/check-membership&user_id=${this.mswaliUserID}`,
      );
      if (isTeamMemberResponse.status) {
        this.isTeamMember = true;
        this.memberTeam = isTeamMemberResponse.data;
      } else {
    ...
    async joinTeam(teamLeaderNumber) {
      try {
